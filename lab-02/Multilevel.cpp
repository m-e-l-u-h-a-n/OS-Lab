...
struct Process {
	int pid, burst, wait, arr, fin, tt, st, prior, rem;
}p[N];
...
int main() {
	...
	int n;
	cin >> n;
	cout << "Enter the Process Type (System/User), arrival time and burst time of each process : " << endl;
	cout << "Type (1/2) Arrival Time\tBurst Time\n";
	int mx = 0;
	for (int i = 0; i < n; i++) {
		p[i].pid = i;
		cin >> p[i].prior >> p[i].arr >> p[i].burst;
		p[i].rem = p[i].burst;
		p[i].st = -1;
		mx += p[i].burst;
	}
